...
void AofLogger::log(Command cmd, const std::string &key,
...
void AofLogger::replay(ReplayCallback callback) {
  ...
  while (infile.peek() != EOF) {
    Header header;
    if (!infile.read(reinterpret_cast<char *>(&header), HEADER_SIZE))
      break;
    ...
    if (header.key_len > 0)
      infile.read(&key[0], header.key_len);
    if (header.value_len > 0)
      infile.read(&value[0], header.value_len);
