...
std::vector<uint8_t>
handle_request(ShardedCache<std::string, std::string> &cache, AofLogger &aof,
  ...
  std::string key(reinterpret_cast<const char *>(data.data() + HEADER_SIZE),
                  header.key_len);
  std::string value;
  if (header.value_len > 0) {
    value.assign(reinterpret_cast<const char *>(data.data() + HEADER_SIZE +
                                                header.key_len),
                 header.value_len);
  }
  ...
  case Command::STATS: {
    ...
    response_val = "Hits: " + std::to_string(stats.hits) +
...
int main(int argc, char **argv) {
  ...
  aof.replay(
      [&cache](Command cmd, const std::string &key, const std::string &value) {
        if (cmd == Command::SET) {
          cache.put(key, value);
  ...
  server.setHandler(
      [&cache, &aof](const std::vector<uint8_t> &data, size_t &consumed) {
